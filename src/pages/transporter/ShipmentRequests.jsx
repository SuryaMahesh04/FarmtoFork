...
const ShipmentRequests = () => {
    ...
    return (
        <DashboardLayout role="transporter">
            <div className={`flex flex-col md:flex-row h-[calc(100vh-100px)] gap-6 ${isMobile ? 'pb-20' : ''}`}>

                {/* Left Panel: Request List */}
                {(!isMobile || !selectedRequest) && (
                    <div className={`${isMobile ? 'w-full flex-1' : 'w-1/3'} flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden`}>
                        {/* Header & Search */}
                ...
                {(!isMobile || selectedRequest) && (
                    <div className={`${isMobile ? 'w-full flex-1 fixed inset-0 z-50 rounded-none' : 'flex-1 rounded-xl'} bg-white shadow-sm border border-slate-200 overflow-hidden flex flex-col`}>
                        {selectedRequest ? (
                            <>
                                {/* Map Header */}
                                <div className="h-64 bg-slate-100 relative group z-0">
                                    {isMobile && (
                                        <button
                                            ...
                                            className="absolute top-4 left-4 z-[500] p-2 bg-white rounded-full shadow-lg text-slate-600 hover:text-slate-800"
                                        >
                                        ...
                                        <div className="flex gap-2 w-full md:w-auto">
                                            {selectedRequest.transporterStatus === 'pending' && (
                                                <button
                                                    ...
                                                    className="flex-1 md:flex-none px-6 py-2 bg-emerald-600 text-white font-bold text-sm rounded-lg shadow-lg shadow-emerald-200 hover:bg-emerald-700 active:scale-95 transition-all"
                                                >
                                                    Accept Request
                                                </button>
                                            )}
                                            {selectedRequest.transporterStatus === 'accepted' && (
                                                selectedRequest.driver ? (
                                                    <div className="w-full md:w-auto px-4 py-2 bg-blue-50 text-blue-700 font-bold text-sm rounded-lg border border-blue-100 flex items-center justify-center gap-2">
                                                        <User size={16} />
                                                    ...
                                                    <button
                                                        ...
                                                        className="flex-1 md:flex-none px-6 py-2 bg-blue-600 text-white font-bold text-sm rounded-lg shadow-lg shadow-blue-200 hover:bg-blue-700 active:scale-95 transition-all flex items-center justify-center gap-2"
                                                    >
                                                        <User size={18} />
                                                        Assign Driver
                                                    </button>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in z-[1000]">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
                        ...
                        <div className="p-6 space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-2">Select Driver (On Duty)</label>
                                <select
                                ...
                                >
                                    ...
                                    {drivers && drivers.length > 0 ? drivers
                                        .filter(d => d.dutyStatus === 'on-duty')
                                        .map(driver => (
                                            <option key={driver._id} value={driver._id}>
                                                {driver.fullName} {driver.assignedVehicle?.registrationNumber ? `(${driver.assignedVehicle.registrationNumber})` : ''}
                                            </option>
                                        )) : <option disabled>No active drivers found</option>}
                                </select>
                            </div>
